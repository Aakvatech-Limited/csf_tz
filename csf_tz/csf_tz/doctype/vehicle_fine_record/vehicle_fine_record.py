...
def sync_vehicle_fines(number_plate):
	...
	for attempt in range(max_retries):
		try:
			...
			frappe.logger().warning(
				f"[VehicleFine] Connection timeout for {number_plate} " f"after {max_retries} retries"
			)
			return {
			...
			if status in (408,) or status >= 500:
				if attempt < max_retries - 1:
					continue
				frappe.logger().warning(
					f"[VehicleFine] HTTP {status} for {number_plate} " f"after {max_retries} retries"
				)
				return {
					"status": "retryable_error",
	...
	try:
		...
		frappe.log_error(
			...
			message=(f"Non-JSON response for {number_plate}: " f"{response.text[:500]}"),
		)
		return {
			"status": "error",
