...
def get_kcb_token():
    ...
    response = requests.post(config.token_url, headers=auth_header)  # Request a new token

    if response.status_code == 200:
        token_data = response.json()  # Parse the response
        token = token_data.get("access_token")  # Extract the token
        expires_in = int(token_data.get("expires_in", 3600))  # Extract expiry time (default 1 hour)
...
def upload_encrypted_file(doc):
    ...
    files = {
        "file": (file_doc.file_name, file_content, "application/octet-stream"),  # File data
