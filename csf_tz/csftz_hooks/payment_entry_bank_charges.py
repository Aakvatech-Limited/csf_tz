...
def get_bank_charges_account(company):
    """Fetch the Default Bank Charges Account for the company"""
...
def validate_bank_charges_account(payment_entry, method):
    ...
    if payment_entry.bank_charges > 0:
        get_bank_charges_account(payment_entry.company) 
...
def create_bank_charges_journal(payment_entry, method):
    ...
    if payment_entry.bank_charges > 0:
        ...
        bank_charges_account = get_bank_charges_account(company)  # Fetches the account
        ...
        journal_entry = frappe.get_doc({
            ...
            "user_remark": payment_entry.bank_charges_description or f"Bank charges for Payment Entry {payment_entry.name}",
        ...
        # Link Journal Entry to Payment Entry
